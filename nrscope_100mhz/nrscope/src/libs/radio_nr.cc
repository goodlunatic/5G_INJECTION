...
int Radio::RadioRxThread()
{
  RadioInitandStartRx("output.fc32");
  return SRSRAN_SUCCESS;
}
...
int Radio::RadioInitandStartRx(const char* filename)
{
  srsran_assert(raido_shared->init(rf_args, nullptr) == SRSRAN_SUCCESS,
                "Failed Radio initialisation");
  ...
  // 设置中心频率
  radio->set_rx_freq(0, rf_args.dl_freq);
  ...
  printf("RX start\n");
  printf("freq = %.3f MHz\n", rf_args.dl_freq/1e6);
  printf("srate = %.2f Msps\n", rf_args.srate_hz/1e6);
  printf("gain = %.1f dB\n", rf_args.rx_gain);

  uint32_t nsamples = 4096;
  nsamples = (uint32_t)(rf_args.srate_hz / 1000.0f / SRSRAN_NOF_SLOTS_PER_SF_NR(ssb_scs));


  cf_t* buffer = srsran_vec_cf_malloc(nsamples);

  srsran::rf_buffer_t rf_buffer;
  rf_buffer.set_nof_samples(nsamples);
  ...
  while (true) {
    ...
    if (!radio->rx_now(rf_buffer, ts)) {
      printf("rx error\n");
      break;
    }

    fwrite(buffer, sizeof(cf_t), nsamples, fp);
  }
