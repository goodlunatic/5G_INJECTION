...
// Configure the parameter for each slot.
int srsran_ue_dl_nr_init_nrscope(srsran_ue_dl_nr_t* q, cf_t* input[SRSRAN_MAX_PORTS], const srsran_ue_dl_nr_args_t* args, srsran_ue_dl_nr_sratescs_info arg_scs)
{
  if (!q || !input || !args) {
    return SRSRAN_ERROR_INVALID_INPUTS;
  }
  ...
  for (uint32_t i = 0; i < q->nof_rx_antennas; i++) {
    ...
    // all zeros
    fft_cfg.out_buffer = q->sf_symbols[i];
    srsran_ofdm_rx_init_cfg_nrscope(&q->fft[i], &fft_cfg, (int)arg_scs.scs);
  }
...
// we are using the different scs setting, could be unnecessary but it makes me feel safe.
int srsran_ue_dl_nr_set_carrier_nrscope(srsran_ue_dl_nr_t* q, const srsran_carrier_nr_t* carrier, srsran_ue_dl_nr_sratescs_info arg_scs)
{
  if (srsran_pdsch_nr_set_carrier(&q->pdsch, carrier) < SRSRAN_SUCCESS) {
    return SRSRAN_ERROR;
  }
  ...
  if (carrier->nof_prb != q->carrier.nof_prb) {
    for (uint32_t i = 0; i < q->nof_rx_antennas; i++) {
      srsran_ofdm_cfg_t cfg     = {};
      cfg.nof_prb               = carrier->nof_prb;
      cfg.symbol_sz             = srsran_symbol_sz_from_srate(arg_scs.srate, arg_scs.scs);
      cfg.cp                    = SRSRAN_CP_NORM;
      cfg.keep_dc               = true;
      cfg.rx_window_offset      = UE_DL_NR_FFT_WINDOW_OFFSET;
      cfg.phase_compensation_hz = carrier->dl_center_frequency_hz;

      srsran_ofdm_rx_init_cfg_nrscope(&q->fft[i], &cfg, (int)arg_scs.scs);
    }
  }
...
void srsran_ue_dl_nr_estimate_fft_nrscope(srsran_ue_dl_nr_t* q, 
...
{ 
  ...
  for (uint32_t i = 0; i < SRSRAN_UE_DL_NR_MAX_NOF_CORESET; i++) {
    if (q->cfg.coreset_present[i]) {
      // we need to do the processing for each slot
...
static int ue_dl_nr_find_dci_ncce_nrscope(srsran_ue_dl_nr_t*     q,
                                  srsran_dci_msg_nr_t*   dci_msg,
                                  srsran_pdcch_nr_res_t* pdcch_res,
                                  uint32_t               coreset_id,
                                  uint16_t*              rnti_list,
                                  uint32_t               rnti_list_length)
{
  ...
  // Decode PDCCH
  if (srsran_pdcch_nr_decode_with_rnti_nrscope(&q->pdcch, q->sf_symbols[0], 
      q->pdcch_ce, dci_msg, rnti_list, rnti_list_length, pdcch_res) < SRSRAN_SUCCESS) {
  // if (srsran_pdcch_nr_decode(&q->pdcch, q->sf_symbols[0], q->pdcch_ce, dci_msg, pdcch_res) < SRSRAN_SUCCESS) {
    ERROR("Error decoding PDCCH");
    return SRSRAN_ERROR;
  }
...
static int ue_dl_nr_find_dci_ncce_nrscope_dciloop(srsran_ue_dl_nr_t*     q,
                                  srsran_dci_msg_nr_t*   dci_msg,
                                  srsran_pdcch_nr_res_t* pdcch_res,
                                  uint32_t               coreset_id,
                                  uint16_t               rnti)
                                ...
                                srsran_rnti_type_t           rnti_type)
{
  ...
  // printf("search_space->nof_formats: %u\n", search_space->nof_formats);
  ...
  for (uint32_t format_idx = 0; format_idx < SRSRAN_MIN(search_space->nof_formats, SRSRAN_DCI_FORMAT_NR_COUNT);
    ...
    for (uint32_t i = 0; i < dci_sizes_count && !skip; i++) {
      // printf("dci_size_count[%u]: %u, dci_nof_bits: %u\n", i, dci_sizes[i], dci_nof_bits);
...
static int ue_dl_nr_find_dci_ss_nrscope(srsran_ue_dl_nr_t*           q,
                                const srsran_slot_cfg_t*     slot_cfg,
                                const srsran_search_space_t* search_space,
                                uint16_t*                    rnti_list,
                                uint32_t                     rnti_list_length,
                                srsran_rnti_type_t           rnti_type)
{
  ...
  for (uint32_t format_idx = 0; format_idx < SRSRAN_MIN(search_space->nof_formats, SRSRAN_DCI_FORMAT_NR_COUNT);
    ...
    for (uint32_t L = 0;
      ...
      uint32_t candidates[SRSRAN_SEARCH_SPACE_MAX_NOF_CANDIDATES_NR] = {};
      int      nof_candidates                                        = srsran_pdcch_nr_locations_coreset(
                                                      coreset, search_space, rnti_list[0], L, SRSRAN_SLOT_NR_MOD(q->carrier.scs, slot_cfg->idx), candidates);
      if (nof_candidates < SRSRAN_SUCCESS) {
        ERROR("Error calculating DCI candidate location");
        return SRSRAN_ERROR;
      }
      ...
      for (int ncce_idx = 0; ncce_idx < nof_candidates && q->dl_dci_msg_count < SRSRAN_MAX_DCI_MSG_NR; ncce_idx++) {
        ...
        // Find and decode PDCCH transmission in the given ncce
        srsran_pdcch_nr_res_t res = {};
        if (ue_dl_nr_find_dci_ncce_nrscope(q, &dci_msg, &res, coreset_id, rnti_list, rnti_list_length) < SRSRAN_SUCCESS) {
          return SRSRAN_ERROR;
        }
        // printf("res.crc: %u\n", res.crc);
        // printf("dci_msg: %u\n", dci_msg.ctx.rnti);
        ...
        // Detect if the DCI is the right direction
        if (!srsran_dci_nr_valid_direction(&dci_msg)) {
          // printf("right direction, try to decode\n");
          // Change grant format direction
          switch (dci_msg.ctx.format) {
            case srsran_dci_format_nr_0_0:
              // printf("format 0_0\n");
...
static int ue_dl_nr_find_dci_ss_nrscope_dciloop(srsran_ue_dl_nr_t*           q,
                                const srsran_slot_cfg_t*     slot_cfg,
                                const srsran_search_space_t* search_space,
                                uint16_t                     rnti,
                                srsran_rnti_type_t           rnti_type)
    ...
    for (uint32_t L = 0;
      ...
      for (int ncce_idx = 0; ncce_idx < nof_candidates && q->dl_dci_msg_count < SRSRAN_MAX_DCI_MSG_NR; ncce_idx++) {
        ...
        if (!srsran_dci_nr_valid_direction(&dci_msg)) {
          ...
          switch (dci_msg.ctx.format) {
            case srsran_dci_format_nr_0_0:
              // printf("0_0 to format 1_0\n");
              dci_msg.ctx.format = srsran_dci_format_nr_1_0;
              break;
            case srsran_dci_format_nr_0_1:
              // printf("0_1 to format 1_1\n");
              dci_msg.ctx.format = srsran_dci_format_nr_1_1;
              break;
            case srsran_dci_format_nr_1_0:
              // printf("1_0 to format 1_0\n");
              dci_msg.ctx.format = srsran_dci_format_nr_0_0;
              break;
            case srsran_dci_format_nr_1_1:
              // printf("1_1 to format 0_1\n");
              dci_msg.ctx.format = srsran_dci_format_nr_0_1;
              break;
            default:
              // printf("unknown format type, continue...\n");
                                ...
                                uint32_t                 nof_dci_msg)
{
  ...
  if (q->cfg.ra_search_space_present && rnti_type == srsran_rnti_type_ra) {
    ...
    for (uint32_t i = 0; i < SRSRAN_UE_DL_NR_MAX_NOF_SEARCH_SPACE && q->dl_dci_msg_count < nof_dci_msg; i++) {
      ...
      // &q->cfg.search_space[0].formats[1]. = 39;
...
int srsran_ue_dl_nr_find_dl_dci_nrscope(srsran_ue_dl_nr_t*       q,
                                const srsran_slot_cfg_t* slot_cfg,
                                uint16_t*                rnti_list,
                                uint32_t                 rnti_list_length,
                                srsran_rnti_type_t       rnti_type,
                                ...
                                uint32_t                 nof_dci_msg)
{
  ...
  if (q->cfg.ra_search_space_present && rnti_type == srsran_rnti_type_ra) {
    // Find DCIs in the RA search space
    int ret = ue_dl_nr_find_dci_ss_nrscope(q, slot_cfg, &q->cfg.ra_search_space, rnti_list, rnti_list_length, rnti_type);
    if (ret < SRSRAN_SUCCESS) {
      ERROR("Error searching RAR DCI");
      return SRSRAN_ERROR;
    }
...
int srsran_ue_dl_nr_find_dl_dci_nrscope_dciloop(srsran_ue_dl_nr_t*       q,
                                                const srsran_slot_cfg_t* slot_cfg,
                                                uint16_t                 rnti,
                                                srsran_rnti_type_t       rnti_type,
                                                srsran_dci_dl_nr_t*      dci_dl_list,
                                                uint32_t                 nof_dci_msg)
{
  ...
  //   for (uint32_t pdcch_idx = 0; pdcch_idx < q->pdcch_info_count; pdcch_idx++) {
  //     const srsran_ue_dl_nr_pdcch_info_t* info = &(q->pdcch_info[pdcch_idx]);
  //     if(info->measure.norm_corr >= q->pdcch_dmrs_corr_thr){
  //       printf("info->measure.norm_corr: %f\n", info->measure.norm_corr);
  //       need_search = true;
  //       break;
  //     }
  //   }
  //   // could be some unknown rnti scrambled dci in this slot  
  //   // if(need_search){
  //   //   printf("is it a si-rnti?\n");
  //   //   // if it's si-rnti, skip
  //   //   q->pdcch_info_count = 0;
  //   //   uint16_t si_rnti[] = {0xffff}; 
  //   //   for (uint32_t i = 0; i < SRSRAN_UE_DL_NR_MAX_NOF_SEARCH_SPACE && q->dl_dci_msg_count < nof_dci_msg; i++) {
  //   //     if(!q->cfg.search_space_present[i]) {
  //   //       continue;
  //   //     }
  //   //     int ret = ue_dl_nr_find_dci_ss_nrscope_dciloop(q, slot_cfg, &q->cfg.search_space[i], si_rnti, 1, srsran_rnti_type_si);
  ...
  //   //     }
  //   //   }
  //   //   if(q->dl_dci_msg_count > 0){
  //   //     printf("si-rnit, not needed in current stage.\n");
  //   //     q->dl_dci_msg_count = 0;
  //   //     need_search = false;
  //   //   }
  //   // }
  //   // not si-rnti, need search
  //   if(need_search){
  //     printf("probably a new rnti?\n");
  //     q->pdcch_info_count = 0;
  //     uint16_t* neighbor_rntis = (uint16_t*) malloc(sizeof(uint16_t) * nof_trial_rnti);
  //     // easy neighbor searching
  //     for (int rnti_nb_id = 0; rnti_nb_id < nof_trial_rnti/2; rnti_nb_id ++){
  //       neighbor_rntis[rnti_nb_id*2] = (uint16_t)((int)rnti_list[*rnti_list_length-1] - rnti_nb_id - 1);
  //       neighbor_rntis[rnti_nb_id*2+1] = (uint16_t)((int)rnti_list[*rnti_list_length-1] + rnti_nb_id + 1);
  //     }
  //     for (uint32_t i = 0; i < SRSRAN_UE_DL_NR_MAX_NOF_SEARCH_SPACE && q->dl_dci_msg_count < nof_dci_msg; i++) {
  //       if(!q->cfg.search_space_present[i]) {
  //         continue;
  //       }
  //       int ret = ue_dl_nr_find_dci_ss_nrscope_dciloop(q, slot_cfg, &q->cfg.search_space[i], neighbor_rntis, nof_trial_rnti, rnti_type);
  ...
  for (uint32_t i = 0; i < dci_msg_count; i++) {
    ...
    // bool exist_rnti = false;
    // for (uint32_t known_id = 0; known_id < *rnti_list_length; known_id ++){
    //   if (q->dl_dci_msg[i].ctx.rnti == rnti_list[known_id]){
    //     exist_rnti = true;
    //     break;
    //   }
    // }
    // if(!exist_rnti){
    //   printf("Found new rnti: %u\n", q->dl_dci_msg[i].ctx.rnti);
    //   *rnti_list_length = *rnti_list_length + 1;
    //   rnti_list[*rnti_list_length] = q->dl_dci_msg[i].ctx.rnti;
...
int srsran_ue_dl_nr_decode_pdsch_nrscope(srsran_ue_dl_nr_t*         q,
                                 const srsran_slot_cfg_t*   slot,
                                 const srsran_sch_cfg_nr_t* cfg,
                                 srsran_pdsch_res_nr_t*     res,
                                 double* phase_diff_first_second_half)
{
  ...
  bool reset_carrier_flag = false;
  if(*phase_diff_first_second_half == 0){
    reset_carrier_flag = true;
  }
  // if(*phase_diff_first_second_half != 0 && reset_carrier_flag){
  //   printf("phase_diff_first_second_half: %f\n", *phase_diff_first_second_half);
  //   for (uint32_t phase_idx = 0; phase_idx < 7; phase_idx++){
  //     q->fft->phase_compensation[phase_idx] = q->fft->phase_compensation[phase_idx] * 
  //                                               (cf_t)cexpf(- I * 2.221039);
  //     // printf("q->fft->phase_compensation[phase_idx]: %f, %f\n", creal(q->fft->phase_compensation[phase_idx]* 
  //     //                                           (cf_t)cexpf(- I * (*phase_diff_first_second_half))), cimag(q->fft->phase_compensation[phase_idx]* 
  //     //                                           (cf_t)cexpf(- I * (*phase_diff_first_second_half))));
  //   }
  // }
  if (srsran_dmrs_sch_estimate_nrscope(&q->dmrs_pdsch, slot, cfg, 
      &cfg->grant, q->sf_symbols[0], &q->chest, phase_diff_first_second_half) < SRSRAN_SUCCESS) {
    return SRSRAN_ERROR;
  }
  // for (uint32_t phase_idx = 0; phase_idx < 14; phase_idx++){
  //   printf("q->fft->phase_compensation[phase_idx]: %f, %f\n", creal(q->fft->phase_compensation[phase_idx]), cimag(q->fft->phase_compensation[phase_idx]));
  // }
  // if(*phase_diff_first_second_half != 0 && reset_carrier_flag){
  //   printf("phase_diff_first_second_half: %f\n", *phase_diff_first_second_half);
  //   for (uint32_t phase_idx = 0; phase_idx < 7; phase_idx++){
