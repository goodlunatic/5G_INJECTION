                                    ...
                                    uint32_t                     candidate)
{
  ...
  // Calculate CORESET bandwidth in physical resource blocks
                           ...
                           srsran_pdcch_nr_res_t*  res)
{
  ...
  uint8_t calculated_rnti[16] = {};
  srsran_bit_unpack(checksum1, &ptr_message, 24);
  srsran_vec_xor_bbb(&message_checksum[8], &oringinal_bits[8], calculated_rnti, 16);
  ptr_message = calculated_rnti;
  // uint32_t rnti_pack = srsran_bit_pack(&ptr_message, 16);
  // printf("rnti should be %u\n", rnti_pack);
...
int srsran_pdcch_nr_decode_with_rnti_nrscope(srsran_pdcch_nr_t*      q,
                           cf_t*                   slot_symbols,
                           srsran_dmrs_pdcch_ce_t* ce,
                           srsran_dci_msg_nr_t*    dci_msg,
                           uint16_t* rnti_list,
                           ...
                           srsran_pdcch_nr_res_t*  res)
{
  ...
  srsran_vec_xor_bbb(&message_checksum[8], &oringinal_bits[8], calculated_rnti, 16);
  ptr_message = calculated_rnti;
  uint16_t rnti_pack = srsran_bit_pack(&ptr_message, 16);
  // printf("rnti should be %u\n", rnti_pack);

  bool si_rnti_flag = false;
  bool ra_rnti_flag = false;

  if(rnti_pack == 0xffff){
    si_rnti_flag = true;
    dci_msg->ctx.rnti = rnti_pack;
    // printf("It's a SI-RNTI scrambled DCI for SIBs, skip...\n");
  }

  for (uint32_t rnti_idx = 0; rnti_idx < rnti_list_length; rnti_idx++){
    if(rnti_pack == rnti_list[rnti_idx]){
      ra_rnti_flag = true;
      dci_msg->ctx.rnti = rnti_pack;
      // printf("It's a RA-RNTI scrambled DCI for RAR, skip...\n");
      break;
    }
  }
  
  // Check the rnti type and checksum_diff.
  // If it's valid dci, the checksum_diff should be less than 65536.
  if(!si_rnti_flag && !ra_rnti_flag && checksum_diff <= 65536){
    res->crc = true;
    dci_msg->ctx.rnti = rnti_pack;
    // printf("It's probably a TC-RNTI, try to decode...\n");
  }else{
    res->crc = false;
  }
                           ...
                           srsran_pdcch_nr_res_t*  res)
{
  ...
  // printf("c=");
  // srsran_vec_fprint_hex(stdout, c, q->K);
  // srsran_vec_fprint_bs(stdout, c, q->K);
  ...
  // printf("c_prime=");
  // srsran_vec_fprint_hex(stdout, c_prime, q->K);
  // srsran_vec_fprint_bs(stdout, c_prime, q->K);
  ...
  // uint32_t checksum_diff = checksum1 > checksum2 ? checksum1 - checksum2 : checksum2 - checksum1;
  // if (checksum_diff < 65535){
  //   printf("checksum_1: %u, checksum_2: %u, checksum diff: %u\n", 
  //     checksum1, checksum2, checksum_diff);
  // }
