                                                ...
                                                srsran_dci_dl_nr_t&        out_dci)
{
  ...
  out_dci.time_domain_assignment = 0;
  if (grant.is_alloc_type0()) {
    out_dci.freq_domain_assignment = grant.rbgs().to_uint64();
  } else {
    ...
    out_dci.freq_domain_assignment = srsran_ra_nr_type1_riv(nof_prb, rb_start, grant.prbs().length());
  }
...
pusch_t& pusch_allocator::alloc_pusch_unchecked(const prb_grant& grant, srsran_dci_ul_nr_t& out_dci)
{
  ...
  if (grant.is_alloc_type0()) {
    ...
    out_dci.freq_domain_assignment = srsran_ra_nr_type1_riv(nof_prb, grant.prbs().start(), grant.prbs().length());
  }
