                         ...
                         srsran_sch_grant_nr_t*           grant)
{
  ...
  //printf("[test yg] m=%d common_time_ra.sliv=%d\n", m, cfg->common_time_ra[m].sliv);
                         ...
                         srsran_sch_grant_nr_t*        grant)
{
  ...
  if (dci_ul->ctx.format == srsran_dci_format_nr_rar) {
    return ra_helper_freq_type1(carrier->nof_prb, 0, dci_ul->freq_domain_assignment, grant);
  }
  ...
  if (cfg->alloc == srsran_resource_alloc_type0) {
    return ra_helper_freq_type0(carrier, cfg, dci_ul->freq_domain_assignment, grant);
  }
