...
public:
                            ...
                            const srsran_dci_ul_nr_t&   dci_ul)
  {
    ...
    //uint32_t tti_tx = (slot_rx.idx - 1 + pusch_cfg.grant.k) % 20480;
    //printf("[test yg] set ul grant for tti_tx=%u slot_rx.idx=%u pusch_cfg.grant.k=%u \n", tti_tx, slot_rx.idx, pusch_cfg.grant.k);
  ...
  bool get_ul_pending_grant(uint32_t tti_tx, srsran_sch_cfg_nr_t& pusch_cfg, uint32_t& pid)
  {
    ...
    //printf("[test yg] get UL pending grant for tti_tx=%u\n", tti_tx);
