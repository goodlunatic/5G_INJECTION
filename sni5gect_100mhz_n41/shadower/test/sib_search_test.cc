...
int main(int argc, char* argv[])
{
  int test_number = 0;
  int input_slot   = 0; 
  std::string input_file = "";  // 存储 -i 的参数

  int opt;
  // 注意：选项字符串是 "c:i:"，表示 -c 和 -i 都需要参数
  while ((opt = getopt(argc, argv, "c:i:s:")) != -1) {
      switch (opt) {
          case 'c':
              test_number = static_cast<int>(strtol(optarg, nullptr, 10));
              break;
          case 'i':
              input_file = optarg;  
              break;
          case 's':
              input_slot = static_cast<int>(strtoul(optarg, nullptr, 10));
              break;
          default:
              std::cerr << "Usage: " << argv[0] << " [-c case] [-i input]" << std::endl;
              return EXIT_FAILURE;
      }
  }
  ...
  switch (test_number) {
    ...
    case 6:
      ...
      break;
    case 7:
      sample_file = "shadower/test/data/n41_pci730_100MHz/sib1.fc32";
      slot_number = 14816;
      half        = 0;
      break;
    default:
