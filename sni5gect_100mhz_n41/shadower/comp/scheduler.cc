...
{
  std::lock_guard<std::mutex> lock(ue_tracker_mutex);

  for (const auto& ue : ue_trackers) {
    if (ue->owns_rnti(rnti)) {
      logger.debug("Ignoring duplicate UE activation for tc-rnti=0x%04x", rnti);
      return;
    }
  }
