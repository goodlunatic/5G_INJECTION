...
{
  std::vector<cf_t> original_symbols(nof_re);
  srsran_vec_cf_copy(original_symbols.data(), gnb_ul.sf_symbols[0], nof_re);
  ...
  const std::array<float, 7> cfo_retry_offsets = {0.0F, 2e-4F, -2e-4F, 5e-4F, -5e-4F, 1e-3F, -1e-3F};
  bool                       decode_ok         = false;
  bool                       retried_cfo       = false;

  for (float delta : cfo_retry_offsets) {
    srsran_vec_cf_copy(gnb_ul.sf_symbols[0], original_symbols.data(), nof_re);
    srsran_vec_apply_cfo(gnb_ul.sf_symbols[0], config.uplink_cfo + delta, gnb_ul.sf_symbols[0], nof_re);

    if (!gnb_ul_pusch_decode(gnb_ul, pusch_cfg, slot_cfg, pusch_res, softbuffer_rx, logger, task->task_idx)) {
      continue;
    }
    if (!pusch_res.tb[0].crc) {
      retried_cfo = retried_cfo || delta != 0.0F;
      continue;
    }
    if (delta != 0.0F) {
      logger.info("PUSCH decode recovered with CFO retry %.6f at slot %u", config.uplink_cfo + delta, slot_cfg.idx);
    }
    decode_ok = true;
    break;
  }
  ...
  if (!decode_ok) {
    if (retried_cfo) {
      logger.debug("PUSCH decode failed after CFO retries at slot %u", slot_cfg.idx);
    }
