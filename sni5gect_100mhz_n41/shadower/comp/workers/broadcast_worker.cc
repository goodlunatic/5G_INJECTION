...
bool BroadCastWorker::decode_rar(srsran::unique_byte_buffer_t& data, uint32_t slot_idx, uint32_t task_idx)
{
  ...
  for (uint32_t i = 0; i < num_subpdus; ++i) {
    ...
    // Only process subPDUs that carry a RAPID (i.e., responses to preambles)
    if (!subpdu.has_rapid()) {
      // This could be a backoff indicator or padding — skip
      continue;
    }

    uint16_t tc_rnti = subpdu.get_temp_crnti();
    if (tc_rnti == SRSRAN_INVALID_RNTI) {
      logger.warning("SubPDU %u: Invalid TC-RNTI, skipping", i);
      continue;
    }

    uint32_t time_advance = subpdu.get_ta();
    uint8_t rapid = subpdu.get_rapid();

    logger.info(CYAN "Found new UE with tc-rnti: 0x%04x (RAPID=%u) slot: %u task: %u" RESET,
                tc_rnti, rapid, slot_idx, task_idx);
  ...
  if (!handled_any) {
    logger.warning("RAR decoded successfully but no valid RAPID-containing subPDU found");
    return false;
  }
  return true;
}
