...
SNI5GECT_DIR = "/home/ubuntu/workarea/git_repo/5G_INJECTION/Sni5Gect-custom/logs"
...
OUTPUT_FILE = "/home/ubuntu/workarea/git_repo/5G_INJECTION/Sni5Gect-custom/logs/decode_rate_report.txt"
...
def parse_sni5gect_dir(sni_dir):
    ...
    按 RNTI 和方向（DL/UL）分类收集 (timestamp, sfn, sf, pdu_bytes) 元组。

    返回结构：
      {
        rnti_int: {
          'DL': [(timestamp, sfn, sf, pdu_bytes), ...],
          'UL': [(timestamp, sfn, sf, pdu_bytes), ...],
    ...
    for fname in pcap_files:
        ...
        try:
            for dlt, data, ts in read_pcap_records(fpath):
                # SNI5GECT 的 pcap 固定为 DLT=149
                pkt = parse_dlt149_packet(data)
                if pkt is None:
                    continue
                # 保留 pcap 时间戳，用于展开超过一个 SFN 周期的长时间采集。
                entry = (ts, pkt['sfn'], pkt['sf'], pkt['pdu_bytes'])
...
def parse_gnb_pcap(gnb_path):
    ...
    try:
        for dlt, data, ts in read_pcap_records(gnb_path):
            pkt = None
            if dlt in (155, 252):
...
def to_slot_in_cycle(sfn, sf):
    """
    将 (SFN, SF) 转换为单个 SFN 周期内的时槽编号。
    ...
    公式：slot = SFN × 10 + SF
    （SFN 最大 1023，对应 slot 最大 10239，15kHz SCS 下约 10.24 秒一个循环周期）
    """
    return sfn * 10 + sf


def infer_slot_duration_ms(packets):
    """
    从 pcap 中的 SF 字段推断时槽长度。

    当前 N3 配置 scs_common=15kHz，SF 字段范围为 0..9，对应 1ms/slot。
    若后续切到 30kHz 且 pcap 记录 0..19，则返回 0.5ms/slot。
    """
    max_sf = max((sf for _, _, sf, _ in packets), default=9)
    return 0.5 if max_sf > 9 else 1.0


def unwrap_slots(packets, ref_ts, ref_slot, slot_duration_ms):
    """
    使用 pcap 时间戳把周期性的 (SFN,SF) 展开为连续时槽编号。

    旧脚本直接用 SFN*10+SF。SFN 每 1024 帧回绕一次，15kHz SCS 下每
    10.24 秒回绕，所以 5 分钟采集会被折叠到同一组 0..10239 的编号里。
    这里用时间戳估算当前包处于第几个 SFN 周期，保留无线侧 slot 编号用于匹配。
    """
    cycle_slots = 10240
    slots = []
    for ts, sfn, sf, _ in packets:
        slot_in_cycle = to_slot_in_cycle(sfn, sf)
        elapsed_slots = (ts - ref_ts) * 1000.0 / slot_duration_ms
        cycle = round((elapsed_slots - (slot_in_cycle - ref_slot)) / cycle_slots)
        slots.append(slot_in_cycle + cycle * cycle_slots)
...
def compute_success_rate(sni_packets, gnb_packets, window=SLOT_WINDOW):
    ...
    all_packets = sni_packets + gnb_packets
    slot_duration_ms = infer_slot_duration_ms(all_packets)
    ref_ts, ref_sfn, ref_sf, _ = min(all_packets, key=lambda p: p[0])
    ref_slot = to_slot_in_cycle(ref_sfn, ref_sf)

    # Step 1：提取 SNI5GECT 唯一时槽，并排序（排序是二分查找的前提）
    sni_abs_sorted = unwrap_slots(sni_packets, ref_ts, ref_slot, slot_duration_ms)

    if not sni_abs_sorted:
        # SNI 没有数据，返回 gNB 唯一时槽数（成功率=0）
        gnb_abs_set = set(unwrap_slots(gnb_packets, ref_ts, ref_slot, slot_duration_ms))
    ...
    gnb_abs_sorted = sorted(set(
        slot
        for slot in unwrap_slots(gnb_packets, ref_ts, ref_slot, slot_duration_ms)
        if sni_min - 50 <= slot <= sni_max + 50
    ))
...
def packet_stats(packets):
    """
    返回 (包数, 持续秒数, 包/秒)，用于报告中判断采集是否覆盖完整测试窗口。
    """
    if not packets:
        return 0, 0.0, 0.0
    timestamps = [ts for ts, _, _, _ in packets]
    duration = max(timestamps) - min(timestamps)
    rate = len(packets) / duration if duration > 0 else 0.0
    return len(packets), duration, rate
...
def print_report(sni_data, gnb_data, output=None):
    ...
    table = PrettyTable()
    table.field_names = [
        "RNTI", "方向", "gNB时槽(窗口)", "SNI5GECT时槽", "匹配时槽", "成功率", "SNI包/秒", "gNB包/秒", "文件"
    ]
    ...
    table.align["匹配时槽"]       = "r"
    table.align["成功率"]         = "r"
    table.align["SNI包/秒"]       = "r"
    table.align["gNB包/秒"]       = "r"
    table.align["文件"]           = "l"
    ...
    for rnti in sni_rntis:
        ...
        if rnti not in gnb_data:
            ...
            for direction in ('DL', 'UL'):
                sni_slots = len(set((s, f) for _, s, f, _ in sni_info[direction]))
                _, _, sni_rate = packet_stats(sni_info[direction])
                table.add_row([rnti, direction, "N/A", sni_slots, "N/A", "N/A", f"{sni_rate:.1f}", "N/A", fname])
            continue
        ...
        for direction in ('DL', 'UL'):
            ...
            _, _, sni_rate = packet_stats(sni_pkts)
            _, _, gnb_rate = packet_stats(gnb_pkts)
            table.add_row([
                rnti, direction, gnb_window, sni_slots, matched, rate_str,
                f"{sni_rate:.1f}", f"{gnb_rate:.1f}", fname
            ])
    ...
    note = (
        ...
        f"  匹配时槽      : 一对一匹配成功的时槽对数（容差 ±{SLOT_WINDOW} 时槽）\n"
         ...
         "  SNI包/秒、gNB包/秒: 原始 pcap 中该方向的 MAC PDU 速率，便于观察是否真实漏解\n"
    )
