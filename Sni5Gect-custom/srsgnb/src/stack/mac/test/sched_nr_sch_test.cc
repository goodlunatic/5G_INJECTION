...
void test_dci_freq_assignment(const bwp_params_t& bwp_params, prb_interval grant, const pdcch_dl_t& pdcch)
{
  ...
  TESTASSERT_EQ(expected_freq_assignment, pdcch.dci.freq_domain_assignment);

  uint32_t st, len;
  srsran_sliv_to_s_and_l(lims.length(), pdcch.dci.freq_domain_assignment, &st, &len);
...
void test_si()
{
  ...
  for (uint32_t i = 0; i < grant_list.size(); ++i) {
    ...
    if (success_expected) {
      ...
      TESTASSERT_EQ(0, pdcch.dci.time_domain_assignment);
