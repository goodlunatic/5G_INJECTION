...
{
  if (running && payload != nullptr) {
    uint32_t subframe_tti = tti;
    if (slots_per_subframe > 1) {
      subframe_tti = tti / slots_per_subframe;
    }
    pcap_pdu_t pdu                     = {};
    ...
    pdu.context_nr.system_frame_number = subframe_tti / 10;
    pdu.context_nr.sub_frame_number    = subframe_tti % 10;
...
{
  pack_and_queue_nr(pdu, pdu_len_bytes, tti, 1, rnti, ue_id, harqid, DIRECTION_DOWNLINK, C_RNTI);
}
...
void mac_pcap_base::write_dl_crnti_nr(uint8_t* pdu,
                                      ...
                                      uint32_t tti)
{
  pack_and_queue_nr(pdu, pdu_len_bytes, tti, 1, crnti, ue_id, harqid, DIRECTION_DOWNLINK, C_RNTI);
}

void mac_pcap_base::write_ul_crnti_nr(uint8_t* pdu, uint32_t pdu_len_bytes, uint16_t rnti, uint8_t harqid, uint32_t tti)
{
  pack_and_queue_nr(pdu, pdu_len_bytes, tti, 1, rnti, ue_id, harqid, DIRECTION_UPLINK, C_RNTI);
}
...
void mac_pcap_base::write_dl_ra_rnti_nr(uint8_t* pdu,
                                        ...
                                        uint32_t tti)
{
  pack_and_queue_nr(pdu, pdu_len_bytes, tti, 1, rnti, ue_id, harqid, DIRECTION_DOWNLINK, RA_RNTI);
}

void mac_pcap_base::write_dl_bch_nr(uint8_t* pdu, uint32_t pdu_len_bytes, uint16_t rnti, uint8_t harqid, uint32_t tti)
{
  pack_and_queue_nr(pdu, pdu_len_bytes, tti, 1, rnti, ue_id, harqid, DIRECTION_DOWNLINK, NO_RNTI);
}

void mac_pcap_base::write_dl_pch_nr(uint8_t* pdu, uint32_t pdu_len_bytes, uint16_t rnti, uint8_t harqid, uint32_t tti)
{
  pack_and_queue_nr(pdu, pdu_len_bytes, tti, 1, rnti, ue_id, harqid, DIRECTION_DOWNLINK, P_RNTI);
}
...
void mac_pcap_base::write_dl_si_rnti_nr(uint8_t* pdu,
                                        ...
                                        uint32_t tti)
{
  pack_and_queue_nr(pdu, pdu_len_bytes, tti, 1, rnti, ue_id, harqid, DIRECTION_DOWNLINK, SI_RNTI);
}
