...
int rf_limesuiteng_open_multi(char* args, void** h, uint32_t num_requested_channels)
{
  ...
  handler->isStreaming = false;
  *h                   = handler;
  handler->tx_count    = num_requested_channels;
  handler->rx_count    = num_requested_channels;
  ...
  handler->state.rx.freq.resize(num_requested_channels);
  handler->state.rx.gain.resize(num_requested_channels);
  handler->state.rx.bandwidth.resize(num_requested_channels);

  handler->state.tx.freq.resize(num_requested_channels);
  handler->state.tx.gain.resize(num_requested_channels);
  handler->state.tx.bandwidth.resize(num_requested_channels);

  handler->state.rf_ports.resize(num_requested_channels);
...
static double rf_limesuiteng_set_srate(void* h, double rate, lime::TRXDir dir)
{
  ...
  if (!handler->isStreaming) {
    handler->state.rf_ports.resize(handler->rx_count);
    handler->state.rf_ports.assign(handler->rx_count, {rate, handler->rx_count, handler->tx_count});
...
int rf_limesuiteng_set_rx_gain_ch(void* h, uint32_t ch, double gain)
{
  ...
  LimePluginContext*        lime    = handler->context;
  if (!handler->isStreaming) {
    handler->state.rx.gain.resize(ch + 1);
...
int rf_limesuiteng_set_tx_gain_ch(void* h, uint32_t ch, double gain)
{
  ...
  LimePluginContext*        lime    = handler->context;
  if (!handler->isStreaming) {
    handler->state.tx.gain.resize(ch + 1);
...
static double rf_limesuiteng_set_freq(void* h, uint32_t ch, double freq, TRXDir dir)
{
  ...
  if (!handler->isStreaming) {
    handler->state.rx.freq.resize(ch + 1);
    handler->state.tx.freq.resize(ch + 1);
...
rf_dev_t srsran_rf_dev_limesuiteng = {"limesuiteng",
                                      ...
                                      rf_limesuiteng_send_timed_multi};
