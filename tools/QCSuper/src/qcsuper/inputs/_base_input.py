...
class BaseInput:
    def __init__(self):
        ...
        self.modules_already_initialized = (
            False  # Whether these instances have been initialized yet
        )
        ...
        with self.input_send_lock:
            with self.event_diag_response_received:
                for nb_tries in range(self.DIAG_MAX_RETRANSMITS + 1):
                    self.send_request(
                        req_opcode, req_payload
                    )  # Dispatched to the underlying input
                    ...
                    response_received = self.event_diag_response_received.wait(
                        self.DIAG_TIMEOUT
                    )
                ...
                resp_opcode, resp_payload = (
                    self.raw_diag_response[0],
                    self.raw_diag_response[1:],
                )
    ...
    def dispatch_received_diag_packet(self, unframed_diag_packet):
        ...
        if opcode == DIAG_LOG_F:  # This is a raw "log" structure
            ...
            DIAG_QSR4_EXT_MSG_TERSE_F,
        ):  # This is a "message" string
            self.dispatch_diag_message(opcode, payload)

        elif opcode == DIAG_EVENT_REPORT_F:
            # Unsolicited event reports from the baseband - ignore
            debug(
                '[<] Ignoring unsolicited event report of length %d'
                % len(payload)
            )

        elif opcode > DIAG_MAX_F:
            # Unknown opcode beyond defined range (e.g. Qualcomm secure
            # diag logs on newer chipsets) - ignore silently
            debug(
                '[<] Ignoring unknown opcode %d of length %d'
                % (opcode, len(payload))
            )
